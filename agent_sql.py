...
def executar_como_chain(pergunta: str, model_name="openai/gpt-oss-20b"):
    ...
    prompt_template = PromptTemplate.from_template(
        ...
        - BUSCA DE TEXTOS (LIKE): Sempre que filtrar por nomes de pessoas, clientes, cidades ou transportadoras na cláusula WHERE (ex: filtro de cliente), utilize o operador LIKE com curingas (ex: p."Cliente" LIKE '%Thiago%') em vez do operador de igualdade (=), pois o banco de dados contém nomes completos (ex: "Thiago Silveira").
        - Colunas que possuem espaços no nome DEVEM ser envolvidas em aspas duplas na query (Ex: "Pedido de Venda").
        - ORDENAÇÃO DE DATAS NO SQLITE: As colunas "Data Hora Recusa" ou "Data" podem estar salvas como texto no formato 'DD/MM/YYYY' ou 'DD/MM/YYYY HH:MM:SS'. Para ordenar cronologicamente de forma correta, use substring: `ORDER BY substr("Data Hora Recusa", 7, 4) || '-' || substr("Data Hora Recusa", 4, 2) || '-' || substr("Data Hora Recusa", 1, 2) DESC`.
        - BUSCA POR ID NA TABELA PEDIDOS_VTEX: Os códigos de pedido na tabela `pedidos_vtex` estão salvos SEM o prefixo "FCN-" (ex: '1642991104922-01'). Ao buscar ou cruzar dados com a tabela `pedidos_vtex`, use `LIKE` para ignorar o prefixo (ex: `WHERE "Order" LIKE '%1642991104922-01'`) ou utilize `REPLACE`.
    ...
    table_info = """
    Tabela rastreio_intelipost: "Pedido de Venda" (string), "Pedido" (string), "Status Transportador" (string), "Data Entrega" (string), "Previsão Entrega Cliente" (string), "Pagina Rastreamento" (string), "e-mail Destinatário" (string), "Celular Destinatário" (string)
